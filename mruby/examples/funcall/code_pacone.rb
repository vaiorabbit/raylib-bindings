...
class Application
  ...
  def update
    return false if Raylib.WindowShouldClose()

    reset_game if Raylib.IsKeyPressed(Raylib::KEY_R)
    dt = Raylib.GetFrameTime()
    @game.update(dt)

    update_objects(dt) unless @game.ready?
    render
    true
  end
  ...
  def update_objects(dt)
    ...
    [@player, @enemy].each do |character|
      character.update(dt)
      character.pos.x = 0 if character.pos.x > @stage.width
      character.pos.x = @stage.width if character.pos.x < 0
    end

    @dots.each do |dot|
      next if dot.eaten?
      next unless Raylib.CheckCollisionCircles(@player.pos, @player.hit_radius, dot.pos, dot.radius)
  ...
  def render
    Raylib.BeginDrawing()
      ...
      if @game.ready?
        ...
        text_widths = [Raylib.MeasureText('GAME OVER', msg_font_size), Raylib.MeasureText('Press R to restart', msg_font_size)]
      ...
      Raylib.DrawText('Left/Right : move player', help_msg_x, help_msg_base_y, 20, Raylib::Color.new(255,255,255,255))
